...
const HealersPage: React.FC = () => {
  ...
  return (
    <IonPage className="sa-page">
      ...
      {/* ── MODAL: VIEW HEALER PROFILE DETAILS ─────────────────────────── */}
      {selectedHealer && !showAssignModal && !showDeactivateModal && !showResetPasswordModal && (
        <div
          style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.6)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}
          onClick={() => setSelectedHealer(null)}
        >
          <div
            style={{ background: '#fff', borderRadius: '16px', padding: '32px', maxWidth: '850px', width: '100%', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)', position: 'relative', maxHeight: '90vh', overflowY: 'auto' }}
            onClick={e => e.stopPropagation()}
          >
            <button
              onClick={() => setSelectedHealer(null)}
              style={{ position: 'absolute', top: '20px', right: '20px', background: '#f1f5f9', border: 'none', width: '32px', height: '32px', borderRadius: '50%', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', color: '#475569' }}
            >
            ...
            <div style={{ display: 'flex', gap: '20px', alignItems: 'center', marginBottom: '24px' }}>
              ...
              <div>
                <h2 style={{ margin: 0, fontSize: '20px', fontWeight: 800, color: '#1e293b' }}>Dr. {selectedHealer.name}</h2>
                <div style={{ fontSize: '12px', color: '#64748b' }}>ID: {selectedHealer.id} • {selectedHealer.branch} Branch</div>
                <div style={{ display: 'flex', gap: '8px', marginTop: '6px', alignItems: 'center' }}>
                  ...
                  <span style={{ fontSize: '11px', color: '#64748b', fontWeight: 700 }}>Joined: {selectedHealer.createdAt}</span>
                </div>
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 2fr', gap: '24px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', borderRight: '1px solid #f1f5f9', paddingRight: '24px' }}>
                <h3 style={{ margin: 0, fontSize: '13px', fontWeight: 800, textTransform: 'uppercase', color: 'var(--ba-color-primary)', borderBottom: '1px solid #f1f5f9', paddingBottom: '6px' }}>Basic Details</h3>
                
                {[
                  { label: 'Gender', val: selectedHealer.gender },
                  { label: 'Date of Birth', val: selectedHealer.dob },
                  { label: 'Mobile Number', val: selectedHealer.phone },
                  { label: 'Email Address', val: selectedHealer.email },
                  { label: 'Address', val: selectedHealer.address },
                  { label: 'Certification', val: selectedHealer.certificationLevel },
                  { label: 'Experience (Years)', val: `${selectedHealer.experience} Years` },
                  { label: 'Specialization', val: selectedHealer.specialization.join(', ') },
                ].map((item, idx) => (
                  <div key={idx} style={{ fontSize: '12px' }}>
                    <div style={{ color: '#94a3b8', fontWeight: 700, textTransform: 'uppercase', fontSize: '9px', marginBottom: '2px' }}>{item.label}</div>
                    <div style={{ color: '#334155', fontWeight: 600 }}>{item.val}</div>
                  </div>
                ))}
              </div>

              {/* Performance & Active caseloads */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                <div>
                  <h3 style={{ margin: 0, fontSize: '13px', fontWeight: 800, textTransform: 'uppercase', color: 'var(--ba-color-primary)', borderBottom: '1px solid #f1f5f9', paddingBottom: '6px', marginBottom: '12px' }}>Caseload & Performance</h3>
                  
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '12px', marginBottom: '16px' }}>
                    <div style={{ background: '#f8fafc', padding: '12px', borderRadius: '8px', textAlign: 'center' }}>
                      <div style={{ fontSize: '16px', fontWeight: 800, color: '#0f5b4b' }}>{selectedHealer.cumulativeHealingCount}</div>
                      <div style={{ fontSize: '9px', color: '#64748b', fontWeight: 700, textTransform: 'uppercase' }}>Healing count</div>
                    </div>
                    <div style={{ background: '#f8fafc', padding: '12px', borderRadius: '8px', textAlign: 'center' }}>
                      <div style={{ fontSize: '16px', fontWeight: 800, color: '#3b82f6' }}>{patients.filter(p => p.assignedHealerId === selectedHealer.id && p.status === 'Active').length}</div>
                      <div style={{ fontSize: '9px', color: '#64748b', fontWeight: 700, textTransform: 'uppercase' }}>Active Patients</div>
                    </div>
                    <div style={{ background: '#f8fafc', padding: '12px', borderRadius: '8px', textAlign: 'center' }}>
                      <div style={{ fontSize: '16px', fontWeight: 800, color: '#10b981' }}>{selectedHealer.completedSessions}</div>
                      <div style={{ fontSize: '9px', color: '#64748b', fontWeight: 700, textTransform: 'uppercase' }}>Sessions done</div>
                    </div>
                    <div style={{ background: '#f8fafc', padding: '12px', borderRadius: '8px', textAlign: 'center' }}>
                      <div style={{ fontSize: '16px', fontWeight: 800, color: '#ef4444' }}>{selectedHealer.urgentFollowUps}</div>
                      <div style={{ fontSize: '9px', color: '#64748b', fontWeight: 700, textTransform: 'uppercase' }}>Urgent Flags</div>
                    </div>
                  </div>
                </div>

                {/* Assigned same branch Patients */}
                <div>
                  <h4 style={{ margin: 0, fontSize: '12px', fontWeight: 800, color: '#475569', marginBottom: '8px' }}>Linked Active Caseload</h4>
                  <div style={{ maxHeight: '180px', overflowY: 'auto', border: '1px solid #f1f5f9', borderRadius: '8px' }}>
                    <table className="sa-table" style={{ width: '100%', margin: 0 }}>
                      ...
                      <tbody>
                          ...
                          <tr key={p.id}>
                            ...
                            <td style={{ textAlign: 'center' }}>
                              ...
                              <button 
                                ...
                                className="sa-btn" 
                                style={{ padding: '2px 8px', fontSize: '10px', background: '#fee2e2', color: '#ef4444', marginLeft: '4px' }}
                              >
                                Remove
                              </button>
                            </td>
                        ...
                        {patients.filter(p => p.assignedHealerId === selectedHealer.id).length === 0 && (
                          <tr>
                            <td colSpan={4} style={{ textAlign: 'center', padding: '16px', color: '#94a3b8', fontSize: '12px' }}>
                ...
                <div>
                  <h4 style={{ margin: 0, fontSize: '12px', fontWeight: 800, color: '#475569', marginBottom: '8px' }}>Recent Treatment Session Log</h4>
                  <div style={{ maxHeight: '180px', overflowY: 'auto', border: '1px solid #f1f5f9', borderRadius: '8px' }}>
                    <table className="sa-table" style={{ width: '100%', margin: 0 }}>
                      <thead>
                        <tr>
                      ...
                      <tbody>
                        {INITIAL_SESSION_HISTORY.filter(s => s.healerId === selectedHealer.id).map((s, i) => (
                          <tr key={i}>
                            <td style={{ fontWeight: 700 }}>{s.sessionNumber}</td>
                            <td>{s.patientName}</td>
                            <td>{s.treatmentType}</td>
                            <td>{s.date}</td>
                            <td>
                              <span style={{ fontSize: '10px', padding: '2px 6px', borderRadius: '4px', background: s.notesStatus === 'Completed' ? '#d1fae5' : '#fee2e2', color: s.notesStatus === 'Completed' ? '#065f46' : '#ef4444', fontWeight: 700 }}>
                                {s.notesStatus}
                              </span>
                            </td>
                          </tr>
